...
class SubscriptionService {
  ...
  Future<void> _loadProducts() async {
    ...
    debugPrint('💳 [SUBSCRIPTION] Requesting IDs: ${SubscriptionProducts.allIds}');
    ...
    try {
      ...
      debugPrint('💳 [SUBSCRIPTION] Query took ${stopwatch.elapsedMilliseconds}ms');
      debugPrint('💳 [SUBSCRIPTION] Found: ${response.productDetails.length}, Not found: ${response.notFoundIDs}');
      
      if (response.notFoundIDs.isNotEmpty) {
        debugPrint('⚠️ [SUBSCRIPTION] Products not found: ${response.notFoundIDs}');
        debugPrint('⚠️ [SUBSCRIPTION] Hint: Ensure IDs match App Store Connect AND StoreKit config');
      }
      
      if (response.error != null) {
        debugPrint('❌ [SUBSCRIPTION] Error loading products: ${response.error}');
        debugPrint('❌ [SUBSCRIPTION] Error code: ${response.error?.code}, source: ${response.error?.source}');
        debugPrint('❌ [SUBSCRIPTION] Error message: ${response.error?.message}');
        debugPrint('❌ [SUBSCRIPTION] Error details: ${response.error?.details}');
      ...
      for (final product in _products) {
        debugPrint('   - ${product.id}: ${product.title} - ${product.price} (${product.rawPrice} ${product.currencyCode})');
      }
      
      if (_products.isEmpty && response.notFoundIDs.isEmpty && response.error == null) {
        debugPrint('⚠️ [SUBSCRIPTION] No products AND no errors — StoreKit config may not be linked to scheme');
      }
    } catch (e, st) {
  ...
  Future<Map<String, dynamic>> runFullDiagnostic() async {
    ...
    debugPrint('   Store available: $storeAvailable (cached: $_isStoreAvailable)');
    ...
    // 3. Loaded products
    diag['loadedProducts'] = _products.map((p) => '${p.id} (${p.price})').toList();
    debugPrint('   Loaded products: ${_products.length}');
    for (final p in _products) {
      debugPrint('     - ${p.id}: ${p.title} — ${p.price} (raw: ${p.rawPrice} ${p.currencyCode})');
    }
    ...
    debugPrint('   Status: $_status | subscribed: $isSubscribed | loading: $_isLoading');
  ...
  Future<void> testProductQuery() async {
    ...
    try {
      ...
      debugPrint('   Error: ${response.error?.code ?? "none"} — ${response.error?.message ?? ""}');
      
      for (final p in response.productDetails) {
        debugPrint('   ✅ ${p.id}: "${p.title}" ${p.price} (${p.rawPrice} ${p.currencyCode})');
        debugPrint('      description: ${p.description}');
      }
      
      if (response.productDetails.isEmpty && response.notFoundIDs.isEmpty && response.error == null) {
        debugPrint('   ⚠️ EMPTY RESPONSE — StoreKit config likely NOT linked to Xcode scheme');
        debugPrint('   💡 Fix: Xcode → Scheme → Run → Options → StoreKit Configuration → select Configuration.storekit');
      }
    } catch (e) {
  ...
  Future<Map<String, bool>> checkStoreHealth() async {
    ...
    try {
      final response = await _iap.queryProductDetails({'shoply_premium_monthly'});
      health['can_query'] = response.error == null;
      health['monthly_found'] = response.productDetails.any((p) => p.id == 'shoply_premium_monthly');
      debugPrint('   Can query: ${health['can_query']} | monthly found: ${health['monthly_found']}');
      if (response.error != null) {
        debugPrint('   Query error: ${response.error!.code} — ${response.error!.message}');
      }
    } catch (e) {
    ...
    debugPrint('   Purchase stream active: ${health['purchase_stream_active']}');
    
    // Check 4: Products already loaded?
    health['products_loaded'] = _products.isNotEmpty;
    debugPrint('   Products in memory: ${_products.length}');
    
    debugPrint('   Overall: ${health.values.every((v) => v) ? "✅ HEALTHY" : "⚠️ ISSUES DETECTED"}');
    debugPrint('');
  ...
  Future<bool> forceReloadProducts() async {
    ...
    debugPrint('   Result: ${success ? "✅ ${_products.length} products loaded" : "❌ No products loaded"}');
    debugPrint('');
  ...
  Future<void> resetSubscriptionState() async {
    ...
    debugPrint('   ✅ Re-initialization complete. Products: ${_products.length}, Status: $_status');
    debugPrint('');
  }
