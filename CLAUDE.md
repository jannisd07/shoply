...
- **Siri Shortcuts / App Intents**: `ios/Runner/AppIntents.swift` (compiled into the `Runner` target as of 2026-07 — it previously existed on disk but was never added to the Xcode project, so it silently had zero effect). Defines `AddItemToListIntent`/`CreateListIntent`/the recipe-browsing intents plus `ShoplyAppShortcuts` (the Siri phrases). Intents hand off to Flutter via `shoply://add-item`, `shoply://create-list`, `shoply://recipes/...` deep links handled in `DeepLinkService`/`app.dart`, not the old App-Group `UserDefaults` pending-queue (that queue lived in a different `UserDefaults` suite with different key names than `SiriService.dart` read, so it never worked — see `FEATURE_IMPLEMENTATION_STATUS.md` Feature 3). `ios/Runner/VoiceAssistantPlugin.swift` (legacy `INIntent`-based donation, `SiriService.dart`'s `com.shoply.app/siri` method-channel path) is **not** part of the Xcode build and has no Dart-side caller — dead/orphaned, intentionally left unbuilt (adding it back would collide with `AppIntents.swift`'s `CreateListIntent` type name in the same target).
