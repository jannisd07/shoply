...
_Last updated: 2026-08-04 (scheduled routine — Feature 8 focus: added a
per-trip savings line to the Shopping History screen's expandable entry row —
"you saved €X on this trip" — on top of the existing lifetime and monthly
savings figures. Closes the "per-trip savings line" Ideas-list entry, the one
half of the tenth-session (2026-07-27) idea the twelfth session's monthly
breakdown explicitly left open. Re-read every feature's own "still open"
notes and the full Ideas list before picking, same discipline every prior
session in this file documents — Features 1–3 remain blocked on real
external/device constraints unchanged from 2026-08-03's audit; Features 4–7's
open items are all explicitly needs-decision or device-gated; Feature 8's own
Ideas list had exactly one `[ ]` entry with no decision flag and Low
complexity/Low risk, unlike its siblings (offers/budget recipe ranking,
assistant memory, premium-gate placement — all needs-decision). See Feature
8's thirteenth-session notes below.)_

_Previously — 2026-08-03 (scheduled routine — Feature 7 focus: extended the
...
addressed to "a Feature 4 session, not this one": the weekly nutrition
recap, diet-challenge streak/adherence awareness, and the lifetime
savings stat surfaced via chat. All three are thin wrappers around
already-existing, already-tested providers/services (`weeklyNutritionSummaryProvider`,
`activeChallengesProvider`, `ShoppingHistoryService.getShoppingHistory`) —
no new computation logic, same "read an existing provider, shape it for
Gemini" pattern `get_personalized_deals` established. See Feature 4's
thirteenth-session notes below.)_

_Previously — 2026-07-30 (scheduled routine — Feature 5 focus: made Avo
...
the same discipline every recent session in this file documents. Features
1–3 unchanged: genuine external constraint (no public shelf-price API) or
device-QA-only. Feature 5's ninth session (07-30) closed its one
buildable-here item and left nothing else non-decision-gated open. Went
looking specifically in Features 6/7/8's own latest-session "Explicitly NOT
done" notes for anything concretely buildable without a device or an owner
decision — found two, both self-addressed to Feature 4 by name: Feature 6's
third session ("No Avo tool for the weekly summary ('how was my week?') —
Feature 4's session is the right place, same precedent as the calorie
tools") and its second session ("Avo/assistant awareness of challenges...
Feature 4/5's own sessions are the right place for that"). A third,
Feature 8's tenth-session Ideas-list entry ("Surface the new lifetime
savings stat... and/or an Avo tool... An Avo tool is a thin read-only
wrapper"), was flagged needs-decision only for the *home-card* placement
half of that idea — its own complexity note says the Avo-tool half needs no
such decision, so it was in scope too. All three follow the exact shape
the 07-29 Feature 4 session used to justify picking up
`get_personalized_deals`: a self-flagged, low-risk, technically-scoped
carryover explicitly addressed to Feature 4, not a fresh product decision.
Confirmed via direct code read of `weekly_nutrition_summary.dart`,
`diet_challenge.dart`/`diet_challenge_service.dart`, and
`shopping_history_screen.dart`'s own savings computation before writing
anything, not trusting the ideas' paraphrase.
...
discipline every recent session documents, looking specifically for
anything genuinely buildable-here (no device, no external API, no owner
decision needed). Features 1–3, 5, 6, 7 are all exhausted on that exact
axis — every remaining item in their own latest session notes is device-QA-
only (widget rendering, real Gemini conversations, onboarding flow on a
real signup), a genuine external constraint (no public shelf-price API,
`mobile_scanner`'s iOS build issue), or explicitly needs-decision (assistant
memory, offers/budget-aware recipe ranking, priorities-aware Avo tone,
"ask which features they want"). Feature 4's own eleventh-session notes
list the identical two needs-decision items as its only remaining gaps.
Rather than force a low-value re-read of an already-thoroughly-audited
feature, went looking for a fresh literal capability gap the way the
2026-07-25/26 Feature 4 sessions did — traced Feature 4's quoted
natural-language-shortcut example **"Find cheap high-protein meals"**
end-to-end and confirmed `search_recipes` had no protein field *and* that a
real "cheap" signal for a multi-recipe browse would require exactly the
per-recipe-per-page-load offer fan-out `RecipeOfferService`'s own doc
comment explicitly flags as "a separately-flagged product decision" — i.e.
this collapses into the already-open, correctly needs-decision "offers/
budget-aware recipe ranking" idea, not a fresh gap. Confirmed via direct
code read rather than assumed, so as not to duplicate work already
correctly gated. That ruled out the FIRST candidate gap this session found,
which is itself worth recording so a future session doesn't retread it.
Pivoted to Feature 8's own required capability ("connect pricing...
shopping lists... history... Avo") and its autonomy rule allowing new
product ideas "if they clearly improve UX or monetization" without needing
owner sign-off (unlike a premium gate). Grepped the codebase for any
existing "you've saved" / savings-recap surface — found none anywhere,
despite `StoreOffer.regularPrice` (the exact data needed) already existing
and already being used for a single-recipe savings figure
(`RecipeOffersResult.knownSavings`). A durable, cross-trip savings total
was a genuine, previously-undocumented gap: real (not fake) since every
number traces back to an actual applied offer price, needs no owner
decision, no device to verify the core logic, and directly serves Feature
8's "growth/retention" mandate. Built it.
...
**Thirteenth session, 2026-07-31 (scheduled routine — this feature's
dedicated session).** See the top-of-file "why Feature 4" note for how this
was found: three self-flagged carryovers sitting in Feature 6's and
Feature 8's own "still open"/Ideas notes, all explicitly addressed to a
future Feature 4 session.

**Before this session:** confirmed via direct code read that none of the
three existed as a chat tool — grepped every `FunctionDeclaration` in
`avo_assistant_service.dart` for `weekly`, `challenge`, and `saving`/`savings`
and found zero matches, not assumed from the idea log's own description.

**What I implemented — three new read-only tools, same "read an existing
provider/service, shape it for Gemini" pattern `get_personalized_deals`
already established (no new computation logic in any of the three):**

1. **`get_weekly_nutrition_summary`** ("how was my week?") — reads
   `weeklyNutritionSummaryProvider` (Feature 6's third session,
   `WeeklyNutritionSummary`), gated by `calorieTrackingEnabledProvider` and a
   configured goal (the same two gates `get_nutrition_status`/
   `setup_nutrition_goal` already use, since calorie tracking is opt-in per
   Feature 7). Returns days logged, average calories/macros vs. targets,
   days within budget, water, logging streak, cooked meals (incl.
   high-protein ones), and weight change when two weigh-ins exist —
   everything the dedicated `WeeklySummaryScreen` already shows, now also
   answerable in chat. Pure shaping extracted to `@visibleForTesting
   summarizeWeeklyNutrition(WeeklyNutritionSummary, NutritionGoal)`.
2. **`get_challenge_status`** ("how's my fasting streak?", "am I still on
   track with no sugar?") — reads `activeChallengesProvider` (Feature 6's
   second session, `DietChallenge`), returns per-challenge type, current
   streak, whether checked in today, total/kept check-ins, adherence rate,
   and days remaining for the fixed-length challenge. Explicitly read-only
   in both the tool description and the empty-state note — it cannot start,
   check in to, or end a challenge (those stay app-only actions in the
   Challenges screen), so it never invites Gemini to attempt a write this
   tool can't perform. Pure shaping in `@visibleForTesting
   summarizeChallengeStatus(List<DietChallenge>)`.
3. **`get_savings_summary`** ("how much have I saved?") — reads
   `ShoppingHistoryService.getShoppingHistory()` and sums
   `item.savingsPerUnit! * item.quantity` across every trip, **byte-for-byte
   the same computation `ShoppingHistoryScreen` already runs** (Feature 8's
   tenth session) — a chat answer and the History screen's "You've saved
   €X" line can never disagree. Reports 0 with an explanatory note (not
   retroactive, only counts items bought at a live offer price) when
   nothing has been saved yet, same "no fake surfaces" discipline every
   other conditional tool in this file follows. Pure shaping in
   `@visibleForTesting summarizeLifetimeSavings(List<ShoppingHistory>)`.

Also added a system-prompt routing paragraph for all three (mirroring the
existing bullet list's style) and a short system-prompt note that
`get_challenge_status` cannot start/modify a challenge, so the model doesn't
try to "confirm" a challenge start through it.

**Design decisions:**
- Reused the exact providers/services the in-app screens already read from
  in all three cases — no new matching, caching, or aggregation logic
  anywhere in this session, consistent with the ideas' own "thin wrapper" /
  "Low complexity" assessments holding up in practice, same as the twelfth
  session's `get_personalized_deals`.
- `get_savings_summary` calls `ShoppingHistoryService().getShoppingHistory()`
  directly (unbounded — the real lifetime, not a capped recent-history
  fetch) rather than going through `ref` — this file already has two other
  call sites doing the same (`_toolSplitTripCost`, `_toolShoppingHistory`),
  so this isn't a new access pattern.
- Did **not** build the Feature 8 Ideas-list home-card half of the savings
  idea (still needs-decision on fifth-card clutter) — only the Avo-tool
  half, which that idea's own write-up separated out as not needing that
  decision.
- Did **not** add a way for Avo to *start* a challenge via chat — the
  brief's Feature 4 scope is "AI can interact with shopping history... use
  pricing data" etc., and a challenge start is a multi-step confirmation
  flow (pick a type, confirm) that the existing Challenges screen already
  handles well; this session scoped to the flagged gap (status awareness),
  not a new capability the idea log never asked for.

**Explicitly NOT done / still open:**
- No live Gemini conversation test confirming the model routes "how was my
  week?" / "how's my streak?" / "how much have I saved?" to the right tool
  and phrases a reply well — same standing "no Gemini API key/device in
  this container" limitation every Feature 4 session has flagged.
...
- `get_weekly_nutrition_summary` and `get_challenge_status` both silently
  do nothing useful for a user who has never used those features (empty
  goal / no active challenge) beyond a note telling Gemini so — matches
  every other optional-feature tool's behavior in this file, not a new gap.

**Files changed (thirteenth session):**
`lib/data/services/avo_assistant_service.dart` (3 new imports for
`DietChallenge`/`WeeklyNutritionSummary`/`diet_challenge_provider.dart`, 3
new `FunctionDeclaration`s, 3 new dispatch cases, 3 new tool methods + 3
`@visibleForTesting` pure shaping methods, system-prompt routing +
read-only-tool note), `test/avo_nutrition_challenges_savings_tools_test.dart`
(new — 7 cases: weekly-summary shaping with/without optional fields,
challenge-status empty/active shaping, savings zero-note/aggregation/
never-negative).

**Checks performed (thirteenth session):** Flutter 3.35.6 downloaded fresh
into `/tmp/flutter`; `env.example.dart`/`firebase_options.example.dart`
copied to their gitignored real paths (not committed) so `flutter analyze`
reflects a properly configured checkout. Full-project `flutter analyze`
before (`git stash`, with the new untracked test file moved aside so the
baseline run doesn't see undefined-method errors against it) **601 issues
(0 errors, 59 warnings, 542 info)** vs. after **601 issues — byte-identical**;
also scoped `flutter analyze` to the touched service file + new test file
together — no issues found. `flutter test` — **68/68 passed** (61
pre-existing + 7 new in `avo_nutrition_challenges_savings_tools_test.dart`,
confirmed individually via the runner's own `+N` counter, not just the
final tally). `pubspec.lock` churn from `flutter pub get` reverted before
committing; the gitignored `env.dart`/`firebase_options.dart` stubs deleted
again afterward, not committed. Traced every new provider/service call
(`weeklyNutritionSummaryProvider`, `nutritionGoalProvider`,
`activeChallengesProvider`, `ShoppingHistoryService.getShoppingHistory`)
...
**Tenth session, 2026-07-27 (scheduled routine — this feature's dedicated
session).** Picked per the walk described at the top of this file — Features
1–7 (and Feature 4 itself) are each exhausted down to device-QA-only,
external, or needs-decision gaps on their own latest session's honest
accounting; re-verified rather than assumed. Feature 4's own quoted
shortcut "Find cheap high-protein meals" looked like a fresh candidate but,
on inspection, collapses into the already-open, correctly needs-decision
"offers/budget-aware recipe ranking" idea (a real per-recipe offer lookup
for a multi-recipe browse is exactly what `RecipeOfferService`'s own doc
comment reserves as "a separately-flagged product decision") — not
attempted, to avoid quietly re-opening a decision that's still pending.

**Before this session:** No surface anywhere in the app told a user how
much money offers had actually saved them, despite the data existing in
principle — `StoreOffer.regularPrice` (and its recipe-scoped sibling
`RecipeIngredientOffer.regularPrice`) already compute a "regular price"
from the marktguru API, and `RecipeOffersResult.knownSavings` already sums
it for a single recipe's ingredient list. But nothing persisted that
regular price once an offer was actually applied to a shopping-list item —
`shopping_items`/`shopping_history_items` only ever stored the discounted
`price`, so a savings figure could never be computed after the fact or
across trips. Confirmed via grep (`saved.*offer`, `savings`, `SavingsCard`)
that no such stat, card, or service existed anywhere in `lib/`.

**What I implemented:**
- **`price_old_value`** (new, nullable `numeric` column on both
  `shopping_items` and `shopping_history_items`, applied live via the
  Supabase MCP tools and mirrored in
  `supabase/migrations/20260727000000_price_old_value_savings_tracking.sql`)
  — the offer's regular price at the moment `price` was set from a live
  offer. Purely additive; never populated for manually-priced or unpriced
  items.
- **`ShoppingItemModel.priceOldValue`** + **`savingsPerUnit`** (new getter:
  `priceOldValue - price` when both are known and the old price is
  genuinely higher, else `null` — never a negative or fabricated saving) —
  full fromJson/toJson/copyWith/props wiring. **`ShoppingHistoryItem`** gets
  the identical field + getter, since a completed trip is where a lifetime
  total is actually computed from.
- **`ItemRepository.addItem`** and **`ItemsNotifier.addItem`** now accept
  `priceOldValue` and write it in both the insert and same-name-merge
  paths — plumbing only, no behavior change for callers that don't pass it.
- **Every offer-apply call site now threads `offer.regularPrice` /
  `offer?.regularPrice` through**, so the value is actually populated in
  practice, not just accepted and ignored:
  `item_offer_sheet.dart` (`_apply`, raw `updateItem` map),
  `select_list_bottom_sheet.dart` (`_addIngredientsToList`,
  `RecipeIngredientOffer`), `avo_nudge_card.dart` (`_addToList`,
  `OfferNudge`), `list_detail_screen.dart` (offer-bar add, `StoreOffer`),
  and `avo_assistant_service.dart`'s `_toolAddRecipeToList` (the eleventh
  Feature 4 session's own offer-enriched chat add). All five already
  computed the offer's regular price for other purposes (a badge, a
  discount %, a savings line) — this only adds persisting it.
- **`ShoppingHistoryService.completeShoppingTrip`** now carries
  `item.priceOldValue` into the `shopping_history_items` insert alongside
  the existing `price`/`price_retailer` passthrough — the point where a
  savings figure becomes durable across trips instead of living only on the
  active list.
- **The stat itself:** `ShoppingHistoryScreen` sums
  `item.savingsPerUnit! * item.quantity` across every history item in every
  trip (lifetime, matching the screen's existing all-time "history_label"
  count rather than the two month-scoped stats, since a cumulative total is
  the more meaningful retention signal and avoids an early-in-the-month
  "€0.00 saved" reading). Rendered as a small accent-colored line
  ("You've saved 12,50 € thanks to offers.", `Icons.savings_outlined` —
  reusing the same icon `price_comparison_nudge_card.dart` and
  `app_priorities.dart`'s "save_money" priority already use) directly under
  the existing stats row, **only when total savings > €0.01** — never an
  empty/zero line, same "no fake surfaces" discipline every other
  conditional card in this file follows. Deliberately *not* added as a
  fourth `_Stat` in the existing stats `Row`: that row is three fixed-width
  columns with no overflow guard, and a fourth would risk clipping on
  narrow screens (no device available in this container to verify
  otherwise) — a plain conditional line carries no such layout risk.
- 2 new EN/DE translation keys (`history_savings_line`).

**Design decisions:**
- Lifetime, not monthly — deliberately different framing from the
  `itemsBought`/`monthEntries.length` stats already in that row, which are
  scoped to the current month. A cumulative "impact" number is the more
  honest and more motivating framing for a savings stat, and matches
  `entries.length`'s existing all-time framing right next to it.
  Reconstructible from history at any time, not double-counted.
- No Avo integration and no home-screen card — scoped strictly to the
  Shopping History screen (this session's chosen feature's own natural
  home for a savings recap) to avoid both scope creep into Feature 4's
  files and the exact "a fifth home card risks clutter" problem the
  high-protein-meals session already reasoned through. Flagged as an idea
  below for whoever wants to extend it.
- Reused `StoreOffer.regularPrice`/`RecipeIngredientOffer.regularPrice`
  exactly as they already existed (parses `oldPrice` or a "Normalpreis:
  X"-style description fallback) — no new price-source logic, only
  persisting a value that was already being computed and shown, just never
  saved.

**Explicitly NOT done / still open:**
- **Not retroactive** — items added or trips completed before this session
  have no `price_old_value`, so the lifetime total only grows from here
  forward. A backfill isn't possible: the regular price was never stored,
  so there's nothing to compute it from after the fact. Worth knowing so a
  "why is this €0 for an active user" report isn't mistaken for a bug.
- **No Avo-chat awareness** ("how much have I saved?" doesn't route
  anywhere) and **no home-screen surface** — both flagged as ideas below,
  deliberately not built here to stay inside this session's scope (see
  Design decisions).
- **No monthly/per-trip breakdown**, only the single lifetime figure — a
  reasonable v1 scope; a "this month you saved €X" or per-trip savings line
  would reuse the same `savingsPerUnit` getter and is a natural, low-risk
  follow-up.
- Not verified on a real device/simulator — no macOS/Xcode in this
  container, the same standing limitation every UI-facing session in this
  file carries. Whether the new line wraps correctly at real device widths,
  and whether real accounts accumulate a meaningful, motivating figure in
  practice (rather than reading as trivial), are both unverified.

**Files changed:**
`supabase/migrations/20260727000000_price_old_value_savings_tracking.sql`
(new, applied), `lib/data/models/shopping_item_model.dart`
(`priceOldValue`, `savingsPerUnit`), `lib/data/models/shopping_history.dart`
(`ShoppingHistoryItem.priceOldValue`, `savingsPerUnit`),
`lib/data/repositories/item_repository.dart` (`addItem` param + insert/merge
writes), `lib/presentation/state/items_provider.dart` (`addItem` param
passthrough), `lib/presentation/screens/lists/widgets/item_offer_sheet.dart`,
`lib/presentation/screens/recipes/widgets/select_list_bottom_sheet.dart`,
`lib/presentation/screens/home/widgets/avo_nudge_card.dart`,
`lib/presentation/screens/lists/list_detail_screen.dart`,
`lib/data/services/avo_assistant_service.dart` (all five: pass
`priceOldValue`/`price_old_value` from the matched offer's `regularPrice`),
`lib/data/services/shopping_history_service.dart` (carry `priceOldValue`
into `shopping_history_items`), `lib/presentation/screens/history/shopping_history_screen.dart`
(lifetime savings computation + conditional UI line),
`lib/core/localization/app_translations.dart` (`history_savings_line`,
EN/DE), `test/savings_tracking_logic_test.dart` (new, 8 cases: both models'
`savingsPerUnit` null/zero/positive edge cases, and the lifetime-aggregation
logic itself — zero-savings trips, mixed priced/unpriced/break-even items,
quantity scaling).
...
properly configured checkout. Full-project `flutter analyze` before (`git
stash`) **601 issues (0 errors, 59 warnings, 542 info)** vs. after **601
issues — byte-identical**; also scoped `flutter analyze` to the 12
new/touched Dart files — the only findings were pre-existing issues in
`list_detail_screen.dart` unrelated to this change (dead code, unused
methods, `print` calls at other lines), confirmed present before this
session's diff too. `flutter test` — **48/48 passed** (40 pre-existing + 8
new in `test/savings_tracking_logic_test.dart`). The new `price_old_value`
columns were verified live against the real `ShoplyAI` Supabase project via
the Supabase MCP tools (`information_schema.columns` confirms both
`numeric`, nullable). `pubspec.lock` churn from `flutter pub get` (this
container's SDK resolves a slightly different lock than the committed one)
reverted before committing; the gitignored `env.dart`/`firebase_options.dart`
stubs deleted again afterward, not committed. **Not run:** an actual
...
**Thirteenth session, 2026-08-04 (scheduled routine — this feature's
dedicated session).** See the top-of-file note for the selection reasoning:
re-read every feature's own "still open" sections and the full Ideas list
directly before picking. Features 1–3 unchanged from the 2026-08-03 audit
(external API constraint / device-QA-only respectively). Feature 4 is
needs-QA only. Features 5–7's single open items are each explicitly
needs-decision (offers/budget recipe ranking; assistant memory; the "which
features do you want" onboarding question, which has no feature-gating system
to hook into yet). Feature 8's own Ideas list has exactly one entry flagged
with no decision gate and Low complexity/Low risk: "Per-trip savings line...
on top of the monthly/lifetime figures" — the twelfth session's own notes
explicitly separated the monthly breakdown (built) from the per-trip breakdown
(left open, "a natural, still-open follow-up").

**Before this session:** The Shopping History screen showed lifetime and
monthly "you've saved €X" figures (tenth/twelfth sessions), both computed via
`totalSavingsOf`. The per-item savings amount was implicitly visible nowhere
— a user expanding one trip's row saw its item list and split-cost section,
but no per-trip total, even though `ShoppingHistoryItem.savingsPerUnit` (and
therefore `totalSavingsOf([thatOneTrip])`) already had everything needed to
compute it. Confirmed by reading `shopping_history_screen.dart`'s actual
`_HistoryRow` widget directly before writing anything.

**What I implemented:**
- `_HistoryRow.build()` now computes `totalSavingsOf([entry])` — reusing the
  exact same pure function that drives the lifetime and monthly figures, so
  all three can never disagree about what counts as a "saving" (only items
  with both a real price and a genuinely higher stored offer regular price).
  No new computation logic; this is purely the existing lifetime/monthly
  pattern applied to a list of one trip, which `totalSavingsOf`'s
  `Iterable<ShoppingHistory>` signature already supported without any change.
- A new conditional row in the expanded section, directly under the item
  list and above the split-cost section, shown only when the trip's savings
  exceed the same `> 0.01` float-noise threshold every other savings figure
  in this file uses: a small `savings_outlined` icon (13px, one size down
  from the 16px lifetime-figure icon, since this is the third and most local
  of three savings figures now on this one screen) + "You saved €X on this
  trip." in the accent color, styled at 12px/w500 to sit clearly below the
  bolder 13px/w600 lifetime line while still reading as a real stat, not
  throwaway metadata.
- One new EN/DE translation key pair: `history_trip_savings_line`.
- Two new test cases in `test/savings_tracking_logic_test.dart`
  (`totalSavingsOf([singleTrip])` group): confirms a single-trip list sums
  only that trip's items (not bleeding in a second trip passed alongside it)
  and returns zero for a trip with no offer-priced items — the same two
  edge cases the lifetime/monthly tests already cover, applied to the
  single-trip usage this UI now relies on.

**Design decisions:**
- Placed the new line inside the already-expanded row (not the collapsed
  summary) — this is a "look closer" detail like the item list and split
  section it sits next to, not a headline stat like the screen-level
  lifetime/monthly figures, which stay visible without expanding anything.
- Did not add this to `get_savings_summary` (Feature 4's Avo tool) or any
  home-screen surface — out of scope for a per-trip UI detail; the existing
  tool is lifetime/monthly-scoped by design (per Feature 4's fourteenth-
  session notes), and a per-trip Avo query ("how much did I save on
  Tuesday's Lidl trip?") is a distinct, narrower ask than anything currently
  requested there.
- Icon size (13px vs. the lifetime line's 16px) and font weight (w500 vs.
  w600) are a deliberate visual hierarchy choice, not an oversight — three
  savings figures now live on one screen (lifetime, monthly, per-trip) and
  only the most prominent one should compete visually with the screen's
  headline stats row.

**Explicitly NOT done / still open:**
- Not verified on a real device/simulator — no macOS/Xcode in this
  container, the same standing limitation every UI-facing session in this
  file carries. Whether the new line reads well next to the split-cost
  section at real device widths, and whether three stacked savings figures
  (lifetime, monthly, per-trip) feels like too much on one screen, are both
  unverified design judgment calls worth a human look.
- No Avo-chat or home-screen surface for the per-trip figure — see Design
  decisions above; this was deliberately scoped to the one screen/row the
  Ideas-list entry named.
- Premium-gating audit remains open and unchanged — this session did not
  touch it, consistent with every prior session's reasoning (needs an
  explicit owner decision).

**Files changed:** `lib/presentation/screens/history/shopping_history_screen.dart`
(per-trip savings computation + new conditional UI row in `_HistoryRow`),
`lib/core/localization/app_translations.dart` (1 new EN/DE key pair),
`test/savings_tracking_logic_test.dart` (2 new cases).

**Checks performed:** Flutter 3.35.6 downloaded fresh into `/tmp/flutter`;
`env.example.dart`/`firebase_options.example.dart` copied to their gitignored
real paths (not committed) so `flutter analyze` reflects a properly
configured checkout. Full-project `flutter analyze` before/after via `git
stash`: baseline **601 issues (0 errors, 59 warnings, 542 info)** → after
**601 issues — byte-identical**. `flutter test` — **all tests passed** (84
total, including the 2 new cases). `pubspec.lock` churn from `flutter pub
get` reverted before committing; the gitignored `env.dart`/`firebase_options.dart`
stubs deleted again afterward, not committed. **Not run:** an actual
Xcode/simulator build, or a live account with a real offer-priced completed
trip to see the new line render end-to-end.

---

## Ideas / Needs My Approval

- [x] IDEA (Avo-tool half DONE 2026-07-31, Feature 4's thirteenth session):
  Surface the new lifetime savings stat (Feature 8's tenth session,
  2026-07-27) beyond the Shopping History screen — a home-screen card
  ("You've saved €48 with Shoply so far") and/or an Avo tool so "how much
  have I saved?" gets a real answer instead of nothing.
  - **Outcome (Avo-tool half):** implemented as `get_savings_summary` —
    reuses `ShoppingHistoryScreen`'s exact lifetime-savings computation
    (`item.savingsPerUnit! * item.quantity` summed across all history), so
    chat and the History screen can never disagree. Details in Feature 4's
    thirteenth-session notes.
  - **Home-card half still NOT done** — this idea's own complexity note
    separated the two: the Avo tool "is a thin read-only wrapper" needing
    no decision, while a home card "needs a placement decision (replace/
    rotate with an existing card vs. add a new one)" because of the
    already-flagged "fifth card" clutter risk. That decision is still open;
    everything below is unchanged and still applies to the home-card half
    only.
  - Why it helps: the number already exists and updates itself for free
    once earned; a home-card would make a purely-passive stat into an
    active retention/delight moment (the chat half already does this now).
  - Expected user value: medium — a satisfying, low-effort "look what the
    app has done for you" moment, the kind of number people share.
  - Expected business/premium value: medium — a strong, honest premium
    upsell candidate later ("unlock savings breakdown by store/category"),
    though this idea itself is just visibility, not gating.
  - Complexity: Low — the computation already exists (and is now reused
    twice); a home card risks the same "fifth card" clutter this session's
    own notes flagged, so it needs a placement decision (replace/rotate
    with an existing card vs. add a new one) more than new logic.
  - Risk: Low technically. The only real risk is home-screen clutter if
    added as a naive sixth proactive card.
...
- [x] IDEA (DONE 2026-08-04, Feature 8's thirteenth session): Per-trip savings
  line (e.g. in the expandable history entry row) on top of the
  monthly/lifetime figures shipped 2026-07-27/2026-08-01 — the "per-trip"
  half of the tenth-session idea that turned out not to be covered by the
  monthly breakdown above.
  - **Outcome:** implemented as a new conditional row inside `_HistoryRow`'s
    expanded section, directly under the item list — "You saved €X on this
    trip." Reuses `totalSavingsOf([entry])` (the exact function driving the
    lifetime/monthly figures, applied to a one-trip list — no new
    computation), styled one visual step quieter than the screen-level
    lifetime line (13px icon vs. 16px, w500 vs. w600) since it's now the
    third savings figure on the same screen. This session made the call on
    the flagged "is a third savings figure too much" question rather than
    waiting further: scoped it to the already-expanded detail view rather
    than the collapsed summary, so it only appears when a user has already
    chosen to look closer at one trip. Details in Feature 8's
    thirteenth-session notes above.
